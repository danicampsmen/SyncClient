...
export class VFSBridge {
  ...
  public static async selectLocalFolder(customPrompt?: string): Promise<string | null> {
    if (this.isNative()) {
     ...
     // Electron renderer (desktop) path via preload bridge
     ...
     // Fallback modern browsers: File System Access API (Chrome/Chromium/Electron)
     try {
       const anyWin = window as any;
       if (anyWin && typeof anyWin.showDirectoryPicker === 'function') {
         try {
           const dirHandle = await anyWin.showDirectoryPicker();
           // DirectoryHandle.name is not a full path, but usable as a hint
           return dirHandle.name || (VFSBridge.getHomeDir() + '/Documentos/' + DEFAULT_LOCAL_DIR_NAME);
         } catch (err: any) {
           logger.warn('showDirectoryPicker cancelado o no disponible:', err?.message || err);
         }
       }
     } catch (err) {
       logger.debug('File System Access API no está disponible o falló:', err);
     }

     // Legacy fallback: invisible <input webkitdirectory> to allow folder selection in browsers
     try {
       const selected = await new Promise<string | null>((resolve) => {
         ...
         input.addEventListener('change', () => {
           try {
             ...
             // Derive the directory path from the first file's webkitRelativePath if present
         ...
         input.click();
         // Timeout fallback in case browser blocks the picker
         setTimeout(() => { if (document.body.contains(input)) { try { document.body.removeChild(input); } catch {} resolve(null); } }, 15000);
       });
       if (selected) return selected;
     } catch (e) {
       logger.debug('Input webkitdirectory fallback falló:', e);
     }

     // Ultimate fallback: suggest Documents default
