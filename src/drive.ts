...
async function sleepWithRetry(res: Response, retryFactory: (token: string) => Promise<Response>, maxAttempts = MAX_RETRY_ATTEMPTS): Promise<Response> {
  ...
  while (attempt < maxAttempts) {
    // R5 Fix: Backoff Exponencial con Jitter Aleatorio (evita colisiones simultáneas)
    const baseDelay = Math.min(RETRY_MAX_BACKOFF_MS, RETRY_BACKOFF_BASE_MS * (2 ** attempt));
    ...
    try {
      ...
      logger.error('Error durante el reintento 429/5xx:', err?.message || err);
      attempt++;
    }
  }
...
export const getFileContent = async (fileId: string, asBlob = false): Promise<string | Blob> => {
  ...
  const doFetch = (t: string) => fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media&acknowledgeAbuse=true`, {
    headers: { Authorization: `Bearer ${t}` },
  });
