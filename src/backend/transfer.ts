...
export const RESUMABLE_UPLOAD_CHUNK_SIZE = 64 * 256 * 1024;
...
export async function downloadToAtomicFile(options: DownloadOptions): Promise<void> {
  ...
  for (let attempt = 1; attempt <= TRANSFER_MAX_ATTEMPTS; attempt++) {
    const tempFilename = `.syncclient-download-${randomUUID().substring(0, 8)}`;
    const temporaryPath = path.join(path.dirname(options.destinationPath), tempFilename);
    ...
    try {
      ...
      writeStream = fsSync.createWriteStream(temporaryPath, { flags: 'wx' });
      ...
      if (error instanceof FileNotFoundError) {
        logger.warn(`[Transfer] Archivo no encontrado en Drive (404) para ${options.destinationPath}; se procesará como borrado.`);
      } else {
