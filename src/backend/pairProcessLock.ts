  ...
  try {
    ...
    if (process.platform === 'linux') {
      try {
        ...
        if (!cmdline.includes('node') && !cmdline.includes('syncclient') && !cmdline.includes('electron')) {
          return false;
        }

        const status = await fs.readFile(`/proc/${pid}/status`, 'utf8');
        const stateLine = status.split('\n').find(l => l.startsWith('State:'));
        if (stateLine && (stateLine.includes('T (stopped)') || stateLine.includes('T (tracing stop)'))) {
          return false;
        }
      } catch {
        // ignore procfs errors
      }
    }
    return true;
  } catch (error) {
