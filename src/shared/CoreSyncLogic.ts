...
export class CoreSyncLogic {
  ...
  public static parseNumberedFilename(filename: string): NumberedFileInfo {
    ...
    if (numberedMatch) {
      const ext = numberedMatch[3].toLowerCase();
      const baseName = `${numberedMatch[1].trim()}.${ext}`;
      const version = parseInt(numberedMatch[2], 10);
      return { isNumbered: true, baseName, version, extension: ext };
    }
  ...
  public static groupAndSortDuplicates<T extends FileGroupItem>(files: T[]): Map<string, Array<T & { version: number; baseName: string }>> {
    ...
    for (const [_, versions] of groups.entries()) {
      versions.sort((a, b) => {
        if (a.md5Checksum && b.md5Checksum && a.md5Checksum.toLowerCase() === b.md5Checksum.toLowerCase()) {
  ...
  public static computeSyncPlan(
    ...
    for (const [relPath, localEntry] of localSnapshot) {
      ...
      if (!localChanged) {
        if (remoteEntry) {
          if (!remoteEntry.id || !remoteEntry.id.trim()) continue;
    ...
    for (const [_, remoteEntry] of remoteSnapshot) {
      ...
      if (remoteEntry.mimeType.startsWith('application/vnd.google-apps.')) continue;
      ...
      if (!localEntry) {
        ...
        if (dbEntry && dbEntry.baseHash != null && dbEntry.remoteId === remoteEntry.id
          ...
          if (remoteEntry.id && remoteEntry.id.trim().length > 0) {
            plan.downloads.push({
              remoteFile: remoteEntry,
              localPath: remoteEntry.name,
    ...
    for (const [relPath, dbEntry] of dbState) {
      ...
      if (dbEntry.isTombstone) {
        if (existsLocally) {
          const localEntry = localSnapshot.get(relPath);
          const tombstoneTime = dbEntry.localMtime || 0;
          if (localEntry && tombstoneTime > 0 && localEntry.mtime > tombstoneTime + 3000) {
            plan.uploads.push({
              localPath: localEntry.rawName || relPath,
              remoteName: localEntry.name,
              remoteId: undefined,
      ...
      if (!existsLocally) {
        ...
        // FIX CRÍTICO: Omitir registros de directorios (fileSize null/0 sin hash) en el plan de borrado
        if ((dbEntry.fileSize === null || dbEntry.fileSize === 0) && !dbEntry.baseHash) {
          continue; // Es una carpeta; su ciclo de vida lo maneja v2SyncDirectoryTree
        }
    ...
    // --- 4. Reconciliación de Movimientos / Renombres Cruzados (Zero-Byte Move) ---
    const moves: Array<{
      remoteId: string;
      oldLocalPath: string;
      newLocalPath: string;
    ...
    for (let i = plan.deleteRemote.length - 1; i >= 0; i--) {
      const del = plan.deleteRemote[i];
      const dbEntry = dbState.get(del.localPath);
      if (!dbEntry || !dbEntry.baseHash) continue;

      const uploadIdx = plan.uploads.findIndex(u => {
        const localEntry = localSnapshot.get(u.localPath);
        return localEntry && localEntry.hash && localEntry.hash.toLowerCase() === dbEntry!.baseHash!.toLowerCase();
      });

      if (uploadIdx !== -1) {
        const matchedUpload = plan.uploads[uploadIdx];
        plan.deleteRemote.splice(i, 1);
        plan.uploads.splice(uploadIdx, 1);

        moves.push({
          remoteId: del.remoteId,
          oldLocalPath: del.localPath,
          newLocalPath: matchedUpload.localPath,
          newName: matchedUpload.remoteName,
          vectorClock: matchedUpload.vectorClock
        });
      }
    }
