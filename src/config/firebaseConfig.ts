...
function readEnv(name: string): string | undefined {
  // Prefer process.env (server/runtime). Fall back to Vite's import.meta.env (frontend).
  if (typeof process !== 'undefined' && process.env && typeof process.env[name] === 'string') {
    const v = process.env[name];
    return v && v.trim().length > 0 ? v : undefined;
  }

  const importMetaWithEnv = typeof import.meta !== 'undefined'
    ? (import.meta as ImportMeta & { env?: Record<string, string | undefined> })
    : undefined;
  const value = importMetaWithEnv && importMetaWithEnv.env ? importMetaWithEnv.env[name] : undefined;
